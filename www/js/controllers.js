  ...
  $scope.sendContactMail = function(){
    cordova.plugins.email.isAvailable(
      function (isAvailable) {
        // alert('Service is not available') unless isAvailable;
        cordova.plugins.email.open({
          to:      'eventos@33club.com.br',
          cc:      '',
          subject: 'Contato pelo App',
          body:    'Poste seu nome seguido da sua dúvida, ou a data da festa seguida da dúvida'
        });
      }
    );
  };
...
.controller('RateAppCtrl', function($scope) {
  ...
  $scope.rateApp = function(){
    if(ionic.Platform.isIOS()){
      ...
      AppRate.preferences.storeAppURL.android = 'market://details?id=com.ionicframework.todo452160';
      AppRate.promptForRating(true);
    }
  };
})
  ...
  $scope.recoverPassword = function(){
    ...
    .then(function(data){
      if(data.status == "error"){
        ...
        $scope.message ="Link para redefinição de senha foi enviada para você. Por favor verifique seu email.";
      }
      $ionicLoading.hide();
    });
  };
...
.controller('HomeCtrl', function($scope, $rootScope, $state, $ionicLoading, PostService) {
  ...
  $scope.bookmarkPost = function(post){
    $ionicLoading.show({ template: 'Confirmado!', noBackdrop: true, duration: 1000 });
...
.controller('PostCategoryCtrl', function($scope, $rootScope, $state, $ionicLoading, $stateParams, PostService) {
  ...
  $scope.bookmarkPost = function(post){
    $ionicLoading.show({ template: 'Confirmando!', noBackdrop: true, duration: 1000 });
    PostService.bookmarkPost(post);
  };
